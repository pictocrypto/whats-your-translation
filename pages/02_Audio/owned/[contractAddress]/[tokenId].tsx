...
export default function TokenPage({ nft, contractMetadata }: Props) {
  ...
  const { contract: nftCollection } = useContract(NFT_COLLECTION_ADDRESS_AUDIO);
  ...
  async function createBidOrOffer() {
    ...
    if (auctionListing?.[0]) {
      ...
      txResult = await marketplace?.offers.makeOffer({
        assetContractAddress: NFT_COLLECTION_ADDRESS_AUDIO,
        tokenId: nft.metadata.id,
        totalPrice: bidValue,
      });
    } else {
        ...
        <div className={styles.container}>
          ...
          <div className={styles.listingContainer}>
            ...
            <iframe
                ...
                title="darkblock"
                src={`https://app.darkblock.io/platform/matic-mumbai/embed/add/${NFT_COLLECTION_ADDRESS_AUDIO}/${nft.metadata.id}`}
              ></iframe>
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  const contract = await sdk.getContract(NFT_COLLECTION_ADDRESS_AUDIO);
...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  const paths = nfts.map((nft) => {
    return {
      params: {
        contractAddress: NFT_COLLECTION_ADDRESS_AUDIO,
        tokenId: nft.metadata.id,
      },
    };
  });
