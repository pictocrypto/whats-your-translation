...
import {
  ETHERSCAN_URL,
  BOOKS_MARKETPLACE_ADDRESS,
  NETWORK,
  BOOKS_COLLECTION_ADDRESS
...
export default function TokenPage({ nft, contractMetadata }: Props) {
  ...
  const { contract: nftCollection } = useContract(BOOKS_COLLECTION_ADDRESS);

  const { data: directListing, isLoading: loadingDirect } =
    useValidDirectListings(marketplace, {
      tokenContract: BOOKS_COLLECTION_ADDRESS,
  ...
  async function createBidOrOffer() {
    ...
    if (auctionListing?.[0]) {
      ...
      txResult = await marketplace?.offers.makeOffer({
        assetContractAddress: BOOKS_COLLECTION_ADDRESS,
        tokenId: nft.metadata.id,
        totalPrice: bidValue,
      });
    } else {
        ...
        <div className={styles.container}>
          ...
          <div className={styles.listingContainer}>
            ...
            {loadingContract || loadingDirect || loadingAuction ? (
              ...
              <>
                <Web3Button
                  contractAddress={BOOKS_MARKETPLACE_ADDRESS}
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  const contract = await sdk.getContract(BOOKS_COLLECTION_ADDRESS);
...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  const paths = nfts.map((nft) => {
    return {
      params: {
        contractAddress: BOOKS_COLLECTION_ADDRESS,
        tokenId: nft.metadata.id,
      },
    };
  });
